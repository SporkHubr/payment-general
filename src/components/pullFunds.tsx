...
export default function Pay(props:Props){
  ...
  const ourAddress = props.ourAddress
  ...
  async function pullFunds(event:React.FormEvent) {
    ...
    ERC20_contract.transferFrom(user_address, ourAddress, amount_wei, {from: user_address})
  ...
  return (
    ...
    <FormControl>
     ...
     <Select id="currency" placeholder="Select currency you want to approve:" onChange={(e) => setAddressContract(e.target.value)} value= {addressContract}  my={3}>
      ...
      <Button type="submit" isDisabled={!currentAccount}>Send</Button>
    </FormControl>
    </form>
  )
}